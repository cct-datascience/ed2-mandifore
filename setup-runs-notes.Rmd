...
pss <- read_table(pss_file, col_types = cols(patch = col_character()))
...
css <- read_table(css_file, col_types = cols(patch = col_character()))
```
...
Currently there are multiple entries for each time, patch, cohort, pft combination.  It doesn't seem like this should be the case (but I'm not sure).  I'll try summarizing to get the median `dbh` and sum `n`.  I think it should also only need a single year, right?
...
pfts <-
  ...
  mutate(PEcAn = case_when(
    ...
    ED == 9 ~ "temperate.Early_Hardwood", 
    ED == 10 ~ "temperate.North_Mid_Hardwood",
    TRUE ~ PEcAn
  ))
