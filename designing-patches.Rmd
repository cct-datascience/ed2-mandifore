...
pss <- 
  ...
  age = c(50, 50, 1), #TODO: not sure about this
...
For debugging, only do one patch/pft at a time:
...
#   pss |> 
#   filter(patch == 1) |>
#   mutate(area = area/sum(area)) #this might not quite work because of rounding ...
# if(sum(round(pss$area, 7)) != 1) stop("adjust the patch areas manually maybe")
# css <- css |> filter(patch == 1, pft == 7)
```
