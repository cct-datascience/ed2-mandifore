...
pss <- 
  ...
  trk = c(1, 1, 1), #TODO: figure this out. Tree fall for forests, pasture for prairie??
...
Writes them to /data/ shared folder.  PEcAn requires that they meet this regexp: `"\\.lat.*lon.*\\.(css|pss|site)"`.  LOL

```{r}
write.table(pss, "/data/sites/generic_patches/generic.lat0lon0.pss", quote = FALSE, row.names = FALSE)
write.table(css, "/data/sites/generic_patches/generic.lat0lon0.css", quote = FALSE, row.names = FALSE)
```


For testing, do just the pine forest

```{r}
pss |> filter(patch == 1) |> 
  write.table("/data/sites/generic_patches/pine.lat0lon0.pss",  quote = FALSE, row.names = FALSE)
css |> filter(patch == 1) |> 
  write.table("/data/sites/generic_patches/pine.lat0lon0.css",  quote = FALSE, row.names = FALSE)
```
